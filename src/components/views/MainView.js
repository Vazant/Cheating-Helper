...
export class MainView extends LitElement {
    ...
    _updateGroqKey(index, value) {
        this._groqKeys = this._groqKeys.map((key, keyIndex) => (keyIndex === index ? value : key));
        this._keyError = false;
    }
    ...
    async _removeGroqKey(index) {
        this._groqKeys = this._groqKeys.filter((_, keyIndex) => keyIndex !== index);
        if (!this._groqKeys.length) this._groqKeys = [''];
        await this._saveGroqKeys();
    }
    ...
    async _handleStart() {
        ...
        if (this._mode === 'byok') {
            if (!this._groqKeys.some(key => key.trim())) {
                this._keyError = true;
                this.requestUpdate();
                return;
            }
                    ...
                    (key, index) => html`
                        <div class="key-row">
                            <input
                                ...
                                @change=${() => this._saveGroqKeys()}
                                class=${this._keyError ? 'error' : ''}
                            />
                            <button class="key-remove" @click=${() => this._removeGroqKey(index)} title="Remove key">×</button>
                        </div>
                    `
                )}
                <button class="key-add" @click=${() => this._addGroqKey()}>Add another key</button>
                <div class="form-hint">
