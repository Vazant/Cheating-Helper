...
export class HistoryView extends LitElement {
    ...
    getProfileNames() {
        return {
            ...
            exam: 'Exam Assistant',
            profile_senior_java_interview: 'Senior Java Interview',
        };
    }

    _getProfileLabel(session) {
        if (session.profileName) return session.profileName;
        if (session.profile) {
    ...
    getSessionPreview(session) {
        ...
        if (session.profile || session.profileName) parts.push(this._getProfileLabel(session));
